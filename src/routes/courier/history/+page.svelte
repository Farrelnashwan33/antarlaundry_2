...
<div class="flex w-full flex-col gap-6">
	...
	>
		...
		<div
			class="rounded-lg border border-green-100 bg-green-50 px-3 py-1.5 text-sm font-bold text-green-600"
		>
			...
			<table class="w-full text-left text-sm whitespace-nowrap">
				...
				>
					<tr>
						...
						<th class="px-2 py-3 text-right font-medium">Completed At</th>
					</tr>
				</thead>
				<tbody class="divide-y divide-surface-100/60">
					{#each finishedDeliveries as delivery}
						<tr class="transition-colors hover:bg-surface-50/50">
							...
							<td
								class="max-w-[200px] truncate px-2 py-4 text-surface-500"
							...
							<td class="px-2 py-4 text-right font-medium text-surface-500">
								{new Date(delivery.updatedAt).toLocaleDateString('id-ID', {
									day: 'numeric',
									month: 'short',
									year: 'numeric'
								})}
