...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  const stats = {
    pending: allOrders.filter(o => o.orderStatus === 'PENDING' || o.orderStatus === 'PICKUP').length,
    inProgress: allOrders.filter(o => ['PROCESSING', 'WASHING', 'DRYING', 'IRONING', 'READY'].includes(o.orderStatus)).length,
...
export const actions: Actions = {
  ...
  acceptOrder: async ({ request, locals }) => {
    ...
    const order = await prisma.order.findUnique({ where: { id: orderId } });
    if (!order || (order.orderStatus !== 'PENDING' && order.orderStatus !== 'PICKUP')) return fail(400);
